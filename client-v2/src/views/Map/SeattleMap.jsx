...
export const MapContextProvider = ({children}) => {
  ...
  const updateMapCenter = (lat, lng) => {
    console.log('hit')
    const latOffset = (parseFloat(lat) + 0.002575); // lowering so entire popup is visible on small screens
  ...
  const findVenue = async (venueName) => {
    allVenues.forEach(v => {
      if(v.name == venueName) {
        setZoom(16);
        const latOffset = (parseFloat(v.lat) + 0.002575); // lowering so entire popup is visible on small screens
