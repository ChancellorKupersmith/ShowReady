...
const reqQueryParamsCleaner = (req, res, next) => {
  try {
    ...
    req.pageSize = Math.min(
      MAX_PAGE_SIZE,
      parseInt(limit) || DEFAULT_PAGE_SIZE
    );
    req.pageNum = Math.max(parseInt(page) - 1, 0);
    req.fromEachArtist = filters.req.artist.fromEach
      ? Math.max(
          0,
          Math.min(
            Number.MAX_SAFE_INTEGER,
            parseInt(filters.req.artist.fromEach)
          )
        )
      : null;
    req.fromEachAlbum = filters.req.album.fromEach
      ? Math.max(
          0,
          Math.min(
            Number.MAX_SAFE_INTEGER,
            parseInt(filters.req.album.fromEach)
          )
        )
      : null;
    req.fromEachGenre = filters.req.genre.fromEach
      ? Math.max(
          0,
          Math.min(
            Number.MAX_SAFE_INTEGER,
            parseInt(filters.req.genre.fromEach)
          )
        )
      : null;
    ...
    req.randomSeed = filters.randomSeed
      ? filters.randomSeed
      : (Date.now() / 1000) % 1;
    next();
  } catch (err) {
...
const eventWhereConditionBuilder = async (req, res, next) => {
  try {
    ...
    if (filters.dateGThan != "")
      whereConditional += `e.EventDate >= '${new Date(
        filters.dateGThan
      ).toUTCString()}' AND `;
    if (filters.dateLThan != "")
      whereConditional += `e.EventDate <= '${new Date(
        filters.dateLThan
      ).toUTCString()}' AND `;
    if (filters.priceGThan != "")
      whereConditional += `e.Price >= '${filters.priceGThan}' AND `;
    if (filters.priceLThan != "")
      whereConditional += `e.Price <= '${filters.priceLThan}' AND `;
    if (filters.ex.date?.dates?.length)
      whereConditional += `e.EventDate NOT IN ('${filters.ex.date.dates.join(
        `', '`
      )}') AND `;
    if (filters.req.date?.dates?.length)
      whereConditional += `e.EventDate IN ('${filters.req.date.dates.join(
        `', '`
      )}') AND `;
    if (filters.ex.date?.eventTimes?.length)
      whereConditional += `e.EventTime NOT IN ('${filters.ex.date.eventTimes.join(
        `', '`
      )}') AND `;
    if (filters.req.date?.eventTimes?.length)
      whereConditional += `e.EventTime IN ('${filters.req.date.eventTimes.join(
        `', '`
      )}') AND `;
    if (filters.ex.event.names.length)
      whereConditional += `e.Name NOT IN ('${filters.ex.event.names.join(
        `', '`
      )}') AND `;
    if (filters.req.event.names.length)
      whereConditional += `e.Name IN ('${filters.req.event.names.join(
        `', '`
      )}') AND `;
    // remove trailing 'AND'
    if (whereConditional != "WHERE ")
      whereConditional = whereConditional.substring(
        0,
        whereConditional.length - 4
      );
    else whereConditional = "";
...
const venueWhereConditionBuilder = async (req, res, next) => {
  try {
    const { filters } = req.body;
    let whereConditional = "WHERE ";
    if (filters.ex.location.venues.length)
      whereConditional += `v.Name NOT IN ('${filters.ex.location.venues.join(
        `', '`
      )}') AND `;
    if (filters.req.location.venues.length)
      whereConditional += `v.Name IN ('${filters.req.location.venues.join(
        `', '`
      )}') AND `;
    if (filters.ex.location.hoods.length)
      whereConditional += `v.Hood NOT IN ('${filters.ex.location.hoods.join(
        `', '`
      )}') AND `;
    if (filters.req.location.hoods.length)
      whereConditional += `v.Hood IN ('${filters.req.location.hoods.join(
        `', '`
      )}') AND `;
...
const artistWhereConditionBuilder = async (req, res, next) => {
  try {
    const { filters } = req.body;
    let whereConditional = "WHERE ";
    if (filters?.ex?.artist?.names?.length)
      whereConditional += `a.Name NOT IN ('${filters.ex.artist.names.join(
        `', '`
      )}') AND `;
    if (filters?.req?.artist?.names?.length)
      whereConditional += `a.Name IN ('${filters.req.artist.names.join(
        `', '`
      )}') AND `;
    ...
    if (filters?.ex?.source?.spotify)
      whereConditional += `a.SpotifyExternalId IS NULL AND `;
    if (filters?.req?.source?.spotify)
      whereConditional += `a.SpotifyExternalId IS NOT NULL AND `;
    // remove trailing 'AND'
    if (whereConditional != "WHERE ")
      whereConditional = whereConditional.substring(
        0,
...
const genreWhereConditionBuilder = async (req, res, next) => {
  try {
    const { filters } = req.body;
    let whereConditional = "WHERE ";
    if (filters?.ex?.genre?.names?.length)
      whereConditional += `g.Name NOT IN ('${filters.ex.genre.names.join(
        `', '`
      )}') AND `;
    if (filters?.req?.genre?.names?.length)
      whereConditional += `g.Name IN ('${filters.req.genre.names.join(
        `', '`
      )}') AND `;
    // remove trailing 'AND'
    if (whereConditional != "WHERE ")
...
const songWhereConditionBuilder = async (req, res, next) => {
  try {
    const { filters, excludedSongIDs } = req.body;
    let whereConditional = "WHERE ";
    if (excludedSongIDs?.length)
      whereConditional += `s.id NOT IN ('${excludedSongIDs.join(
        `', '`
      )}') AND `;
    if (filters?.ex?.song?.names?.length)
      whereConditional += `s.Title NOT IN ('${filters.ex.song.names.join(
        `', '`
      )}') AND `;
    if (filters?.req?.song?.names?.length)
      whereConditional += `s.Title IN ('${filters.req.song.names.join(
        `', '`
      )}') AND `;
    if (filters?.ex?.source?.spotify)
      whereConditional += `s.SpotifyExternalId IS NULL AND `;
    if (filters?.req?.source?.spotify)
      whereConditional += `s.SpotifyExternalId IS NOT NULL AND `;
    if (filters?.ex?.source?.youtube)
      whereConditional += `s.YTUrl IS NULL AND `;
    if (filters?.req?.source?.youtube)
      whereConditional += `s.YTFound = TRUE AND `;
    // whereConditional += `a.spotifyimg IS NOT NULL AND `;
    // remove trailing 'AND'
    if (whereConditional != "WHERE ")
      whereConditional = whereConditional.substring(
...
const albumWhereConditionBuilder = async (req, res, next) => {
  try {
    const { filters } = req.body;
    let whereConditional = "WHERE ";
    if (filters?.ex?.album?.names?.length)
      whereConditional += `al.Title NOT IN ('${filters.ex.album.names.join(
        `', '`
      )}') AND `;
    if (filters?.req?.album?.names?.length)
      whereConditional += `al.Title IN ('${filters.req.album.names.join(
        `', '`
      )}') AND `;
    // remove trailing 'AND'
    if (whereConditional != "WHERE ")
...
// TODO: ONLY JOIN GENRES IF REQ OR EX IN FILTERS OTHERWISE LEFT JOIN TO INCLUDE SONGS WITHOUT GENRE LABEL
/* SQL Query Notes
  ...
  - 
*/
const fetchEventsArtistsData = async (
  ...
  const filterEventsQuery = `
      SELECT
        ...
        v.Name AS Venue, v.ID as VenueID
      FROM (
  ...
  client.release();
  // filterEventsResult.rows);
  // for combinging fetched db songs on artistids
  const uniqueFilteredArtistIDs = [
    ...new Set(filterEventsResult.rows.map((row) => row.artistid)),
  ];
...
// TODO: DEBUG fromEach album logic
const fetchSongsData = async (
  artistEvents,
  uniqueFilteredArtistIDs,
  ...
  let songsList = filterSongsResult.rows.map((row) => {
    ...
    row["genres"] = Object.values(
      artistEvents[row.artistid].reduce((acc, event) => {
  ...
  const fromEachFiltering = () => {
      ...
      songsList.forEach((song) => {
        if (category === "genres") {
          song["genres"].forEach((genre) => {
            if (!visitedFromEachSongs["genres"][genre])
              visitedFromEachSongs[category][genre] = 0;
        ...
        if (!visitedFromEachSongs[category][categoryKey])
          visitedFromEachSongs[category][categoryKey] = 0;
    ...
    if (Object.values(fromEachSongs).length > 0)
      songsList = Object.values(fromEachSongs);
  };
  fromEachFiltering();
  // handle ordering
  const orderSongsList = () => {
    switch (orderBy) {
      ...
      case OrderBys.EVENT_DATE:
        ...
        songsList.sort((a, b) =>
          orderByDesc
            ? new Date(b.events[b.events.length - 1].eventdate) -
              new Date(a.events[a.events.length - 1].eventdate)
  ...
  const paginateSongsList = () => {
    if (orderBy == OrderBys.RANDOM) {
      // manually grab random indexes in songs list
...
const queryPlaylistLength = async (
  artistEvents,
  uniqueFilteredArtistIDs,
  songWhereConditional,
  albumWhereConditional,
...
// TODO: ONLY JOIN GENRES IF REQ OR EX IN FILTERS OTHERWISE LEFT JOIN TO INCLUDE SONGS WITHOUT GENRE LABEL
const fetchVenueEventsData = async (
  eventWhereConditional,
  venueWhereConditional,
  artistWhereConditional,
  genreWhereConditional
  ...
  const filterEventsQuery = `
    SELECT
      ...
      v.Name AS VenueName, v.ID AS VenueID, v.lat AS VenueLat, v.lng AS VenueLng
    FROM (
    ...
    JOIN (
      SELECT v.Name, v.Hood, v.VenueAddress, v.lat, v.lng,
      v.ID FROM Venues v
      ${venueWhereConditional}
    ) AS v ON e.VenueID = v.ID
    JOIN EventsArtists AS ea ON e.ID = ea.EventID AND e.EventDate = ea.EventDate
...
songsRouter.post(
  ...
  eventWhereConditionBuilder,
  venueWhereConditionBuilder,
  artistWhereConditionBuilder,
  genreWhereConditionBuilder,
  songWhereConditionBuilder,
  albumWhereConditionBuilder,
  async (req, res, next) => {
    try {
      const [artistEvents, uniqueFilteredArtistIDs] =
        await fetchEventsArtistsData(
...
songsRouter.post(
  ...
  async (req, res, next) => {
    try {
      ...
      //   return acc;
      // }, {});
      const events = await fetchVenueEventsData(
        req.eventWhereConditional,
        req.venueWhereConditional,
        req.artistWhereConditional,
        req.genreWhereConditional
