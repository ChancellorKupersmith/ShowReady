...
const getAccessToken = async (req, res, next) => {
  try {
    ...
    if (!response.ok) {
      ...
      console.error(
        "Failed fetching youtube access token for client, " + message
      );
      throw new Error();
    }
...
googleApiRouter.post("/new_playlist", async (req, res, next) => {
  ...
  try {
    ...
    if (accessToken.length === 0 || playlistName.length === 0)
      throw new Error(
        "Missing required params google_access_token and playlistName"
      );
    const payload = {
      snippet: {
        title: playlistName,
...
googleApiRouter.post("/add_playlist_track", async (req, res, next) => {
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        "missing required params playlistId, videoId, or accessToken"
      );
    const payload = {
      snippet: {
...
googleApiRouter.get("/client_api_key", async (req, res) =>
  res.json({ apiKey: process.env.GOOGLE_MAPS_API_KEY_CLIENT })
);
